...
void FixedFunctionGL::SetContextFlags()
{
    ...
    // iOS uses OpenGL ES.
    // This renderer uses the fixed-function pipeline,
    // so use OpenGL ES 1.1 instead of desktop OpenGL 1.3.
    ...
    // Desktop OpenGL 1.3 compatibility context.
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 1);
...
void FixedFunctionGL::ToggleVertexAttribute(u8 attr, bool enable)
{
    if (attr & VERTEX_ATTR_TEX_COORD)
    {
        if (enable)
        {
            ...
            g_glFuncTable.glEnableClientState(
        ...
        else
        {
            ...
            g_glFuncTable.glDisableClientState(
                GL_TEXTURE_COORD_ARRAY
            );
        }
    }
...
void FixedFunctionGL::SetAttributePointer(
    VertexAttributeArrays attr,
    std::size_t stride,
    void *ptr)
{
    switch (attr)
    ...
    case VERTEX_ARRAY_TEX_COORD:
        g_glFuncTable.glTexCoordPointer(
            2,
            GL_FLOAT,
            stride,
            ptr
...
void FixedFunctionGL::SetColorOp(
    TextureOpComponent component,
    ColorOp op)
{
    const GLenum opEnums[3] = {
        GL_MODULATE,
        GL_ADD,
        GL_REPLACE
    };
...
void FixedFunctionGL::SetTransformMatrix(
...
void FixedFunctionGL::SetViewport(
    i32 x,
    i32 y,
    i32 width,
    i32 height)
...
{
    g_glFuncTable.glGetFloatv(
        GL_DEPTH_RANGE,
        depthRange
    );
}

void FixedFunctionGL::SetClearColor(
    f32 r,
    f32 g,
    f32 b,
    f32 a)
...
void FixedFunctionGL::SetDepthRange(
    f32 nearPlane,
    f32 farPlane)
{
    g_glFuncTable.glDepthRangef(
        nearPlane,
        farPlane
    );
}
...
{
    g_glFuncTable.glDeleteTextures(
        1,
        (GLuint *)&handle
    );
}

void FixedFunctionGL::SetTextureImage(
    u32 width,
    u32 height,
    PixelFormat fmt,
    PixelDataType type,
...
void FixedFunctionGL::SetTextureSubImage(
    i32 xoffset,
    i32 yoffset,
    i32 width,
    i32 height,
...
void FixedFunctionGL::ReadPixels(
    i32 x,
    i32 y,
    i32 width,
    i32 height,
